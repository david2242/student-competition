...
describe('UserService', () => {
  ...
  it('should get the users', (done) => {
    service.getAllUsers().subscribe(data => {
      expect(data).toEqual(stubGetAllResponse);
      done();
    });
    const req = httpTesting.expectOne(`${service.apiUrl}/api/user`);
  ...
  it('should get one user', (done) => {
    service.getUser('1').subscribe(data => {
      expect(data).toEqual(stubGetResponse);
      done();
    });
    const req = httpTesting.expectOne(`${service.apiUrl}/api/user/1`);
    expect(req.request.method).toBe('GET');
